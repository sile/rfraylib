...
impl<'a> Monitor<'a> {
    ...
    pub fn get_size(&self) -> Size {
        let width = unsafe { raylib4_sys::GetMonitorWidth(self.index) } as f32;
        let height = unsafe { raylib4_sys::GetMonitorHeight(self.index) } as f32;
        Size { width, height }
    }

    /// Get monitor physical size in millimetres.
    pub fn get_physical_size(&self) -> Size {
        let width = unsafe { raylib4_sys::GetMonitorPhysicalWidth(self.index) } as f32;
        let height = unsafe { raylib4_sys::GetMonitorPhysicalHeight(self.index) } as f32;
        Size { width, height }
    }
